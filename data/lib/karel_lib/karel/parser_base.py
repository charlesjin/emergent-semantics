...
class Parser(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._run_with_trace = run_with_trace
        self._trace = None
        self.trace_scope = None
        if not run_with_trace:
            self.trace = nop
            self.retrace = nop
            self.enter_context = nop
            self.exit_context = nop
            self.reset_trace = nop
    ...
    def trace(self, state):
        self.trace_scope[-1] += 1
        key = tuple(self.trace_scope)
        traces = self._trace.get(key, [])
        traces.append(state)
        self._trace[key] = traces
        return key

    def retrace(self, state, key):
        self._trace[key] = state

    def enter_context(self):
        self.trace_scope.append(-1)

    def exit_context(self):
        self.trace_scope.pop()

    def get_trace(self):
        if not self._run_with_trace:
            raise ValueError("Did not run with trace.")
        if not self._trace:
            raise ValueError("No trace... must run first.")
        return self._trace
    ...
    def random_tokens(
        ...
        if start_token == "stmt":
            if depth > stmt_max_depth:
                start_token = "action"
            elif depth < stmt_min_depth:
                start_token = "stmt_stmt"
        ...
        for term in prod.prod:
            if term in self.prodnames:  # need digging
                codes.extend(
                    self.random_tokens(
                        term, depth + 1, stmt_min_depth, stmt_max_depth, **kwargs
                    )
                )
            else:
...
def parser_prompt(parser):
    ...
    arg_parser.add_argument(
        "--world", type=str, default=None, help="Path to world text file"
    )
    arg_parser.add_argument(
